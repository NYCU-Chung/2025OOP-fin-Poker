...
static void printCardsInTwoRows(const vector<Card>& cards) {
    ...
    for (int c = 0; c < 4; c++) {
        ...
        cout << string(padLeft-5, ' ') << idxStr << string(padRight-5, ' ') << "  ";
    }
    cout << "\n\n";
...
void Game::playStage(Player &p) {
    ...
    while (playRounds < 4 && !p.hand().getCards().empty()) {
        ...
        if (c == 1 && playRounds < 4) {
            ...
            for (int id : v) p.hand().removeAt(id);
            ...
            for (int k = 1; k <= 5; ++k) {
                vector<bool> select(hand.size(), false);
                fill(select.begin(), select.begin() + k, true);
                do {
                    vector<Card> candidate;
                    for (int i = 0; i < (int)hand.size(); ++i) {
                        if (select[i]) candidate.push_back(hand[i]);
                    }
                    int sc = ScoreCalculator::calculate(candidate);
                    if (sc > bestScore) {
                        bestScore = sc;
                        bestCombo = candidate;
                    }
...
void Game::ShowLeaderBoard() {
    ...
    for (int i = 0; i < (int)v.size(); i++)
        cout << "Rank " << i + 1 << ": " << v[i].second << ", Score : " << v[i].first << '\n';
}
